...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //初始化EpathmapSDK  I3FBmzhjPe
    [EpathmapServices setAppKey:APIKey];
    ViewController *listVC = [[ViewController alloc] init];
    EpathNavigationController* nav = [[EpathNavigationController alloc] initWithRootViewController:listVC];
    ...
    //监听ibeacon
#if EpathBeaconRegionEnable
    [[EpathIbeaconManager sharedInstance] startMonitoringForRegion];
#endif
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(epathReceiveShareInfo:) name:IpsReceiveShareInfoNotification object:nil];
    return YES;
}
...
- (BOOL)application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<UIApplicationOpenURLOptionsKey, id> *)options {
    ...
    return [[EpathmapServices sharedInstance] application:app openURL:url delegate:[EpathLocationShareHandle sharedInstance]];
    return YES;
}
